...
from nnunet.network_architecture.generic_modular_custom_UNet import GenericAttentionUNet


class ExperimentPlanner3DGenericAttentionUNet_v21(ExperimentPlanner3D_v21):
    """ Planner for the Fully GenericAttentionUNet 3D UNet"""
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        super(ExperimentPlanner3DGenericAttentionUNet_v21, self).__init__(folder_with_cropped_data, preprocessed_output_folder)
        self.data_identifier = "nnUNetData_plans_v2.1"  # "nnUNetData_FabiansResUNet_v2.1"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnUNetPlans_GenericAttentionUNet_v2.1_plans_3D.pkl")
    ...
    def get_properties_for_stage(self, current_spacing, original_spacing, original_shape, num_cases,
        ...
        blocks_per_stage_encoder = GenericAttentionUNet.default_blocks_per_stage_encoder[:len(pool_op_kernel_sizes)]
        blocks_per_stage_decoder = GenericAttentionUNet.default_blocks_per_stage_decoder[:len(pool_op_kernel_sizes) - 1]

        ref = GenericAttentionUNet.use_this_for_3D_configuration
        here = GenericAttentionUNet.compute_approx_vram_consumption(input_patch_size, self.unet_base_num_features,
        ...
        while here > ref:
            ...
            blocks_per_stage_encoder = GenericAttentionUNet.default_blocks_per_stage_encoder[:len(pool_op_kernel_sizes)]
            blocks_per_stage_decoder = GenericAttentionUNet.default_blocks_per_stage_decoder[:len(pool_op_kernel_sizes) - 1]
            here = GenericAttentionUNet.compute_approx_vram_consumption(new_shp, self.unet_base_num_features,
                                                               self.unet_max_num_filters, num_modalities, num_classes,
                                                               pool_op_kernel_sizes, blocks_per_stage_encoder,
                                                               blocks_per_stage_decoder, 2, self.unet_min_batch_size)
        input_patch_size = new_shp

        batch_size = GenericAttentionUNet.default_min_batch_size

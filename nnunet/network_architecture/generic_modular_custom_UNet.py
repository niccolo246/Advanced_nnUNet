...
class ResidualUNet(SegmentationNetwork):
    ...
    use_this_for_2D_configuration = 704116860.5  # 1167982592.0
    use_this_for_3D_configuration = 704116860.5

    use_this_for_batch_size_computation_2D = 1222116860.5  # 1167982592.0
    use_this_for_batch_size_computation_3D = 1222116860.5  # 1152286720.0
...
class DenseUNet(SegmentationNetwork):
    ...
    use_this_for_2D_configuration = 2022116860.5  # 1167982592.0
    use_this_for_3D_configuration = 2022116860.5
...
class InceptionUNet(SegmentationNetwork):
    ...
    use_this_for_2D_configuration = 703116860.5  # 1167982592.0
    use_this_for_3D_configuration = 703116860.5
    use_this_for_batch_size_computation_2D = 2444233721.0  # 1167982592.0
    use_this_for_batch_size_computation_3D = 2444233721.0  # 1152286720.0
    default_base_num_features = 32
...
class GenericUNet(SegmentationNetwork):
    ...
    use_this_for_2D_configuration = 1142116860.5  # 1167982592.0
    use_this_for_3D_configuration = 1142116860.5
...
class GenericSpatialMultiAttentionUNetDecoder(nn.Module):
    def __init__(self, previous, num_classes, num_blocks_per_stage=None, network_props=None, deep_supervision=False,
                 upscale_logits=False):
        super(GenericSpatialMultiAttentionUNetDecoder, self).__init__()
        self.num_classes = num_classes
        self.deep_supervision = deep_supervision
        """
        We assume the bottleneck is part of the encoder, so we can start with upsample -> concat here
...
class GenericSpatialMultiAttentionUNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
                 pool_op_kernel_sizes, conv_kernel_sizes, props, num_classes, num_blocks_per_stage_decoder,
                 deep_supervision=False, upscale_logits=False, max_features=320, initializer=None):
        super(GenericSpatialMultiAttentionUNet, self).__init__()
        self.conv_op = props['conv_op']
        self.num_classes = num_classes
        ...
        self.encoder = GenericAttentionUNetEncoder(input_channels, base_num_features, num_blocks_per_stage_encoder,
                                                   feat_map_mul_on_downscale, pool_op_kernel_sizes, conv_kernel_sizes,
                                                   props, default_return_skips=True, max_num_features=max_features)
        self.decoder = GenericSpatialMultiAttentionUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder, props,
                                                   deep_supervision, upscale_logits)
        if initializer is not None:
            self.apply(initializer)
    ...
    def compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
        ...
        enc = GenericAttentionUNetEncoder.compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
                                                                          ...
                                                                          num_conv_per_stage_encoder,
                                                                          feat_map_mul_on_downscale, batch_size)
        dec = GenericSpatialMultiAttentionUNetDecoder.compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
                                                                          num_classes, pool_op_kernel_sizes,
                                                                          num_conv_per_stage_decoder,
                                                                          feat_map_mul_on_downscale, batch_size)
...
class GenericChannelSpatialAttentionUNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
        ...
        self.encoder = GenericAttentionUNetEncoder(input_channels, base_num_features, num_blocks_per_stage_encoder,
                                                   feat_map_mul_on_downscale, pool_op_kernel_sizes, conv_kernel_sizes,
                                                   props, default_return_skips=True, max_num_features=max_features)
        self.decoder = GenericChannelSpatialAttentionUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder, props,
                                                         deep_supervision, upscale_logits)
        if initializer is not None:
            self.apply(initializer)
